...
- **`chokidar.watch()` misses changes made before its "ready" event** when `ignoreInitial: true` — the initial scan window silently swallows them. `MockRouter.start()` awaits `ready` for this reason; without it, an edit right after startup is lost. (The per-file watchers in `serve()` have the same race, which is why browser tests sleep briefly before editing a fixture.)
