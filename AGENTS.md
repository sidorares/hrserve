...
- **chokidar drops every other change when writes land back to back.** With the default options, writing the same file twice in quick succession produces only one `change` event; a gap of about a second avoids it. Reproducible with plain chokidar, no hrserve involved, so it affects every file type. `awaitWriteFinish: { stabilityThreshold: 50, pollInterval: 10 }` fixes it (and would also stop hrserve reading a half-written file); not applied yet because it adds latency to every patch. Browser tests that edit the same file twice must space the writes out (`betweenWrites()` in `test/browser/integration.test.ts`).
