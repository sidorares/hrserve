...
- **Chromium removed LiveEdit in 2025.** `Debugger.setScriptSource` fails with "setScriptSource functionality no longer available" on current Chromium. The JS patcher treats it as best-effort and relies on the `script-patch` event as the stable contract. A real replacement (e.g. ESM re-import) is an open design question.
